...
O recorte analítico concentra-se em Minas Gerais, com expansão para \textbf{multi-ano (2023 em diante, pós-pandemia)} via concatenação de arquivos \texttt{datatran<ano>.csv}. Cada registro corresponde a uma ocorrência única de acidente. Após revisão metodológica (Pacote~A), restringimos a análise a acidentes \textbf{com vítimas} e passamos a minerar regras \textbf{contexto $\rightarrow$ desfecho} (Fatal vs.\ Ferido), evitando tautologias entre \texttt{classificacao\_acidente} e \texttt{gravidade\_binaria}. Com o arquivo parcial de 2026 (2.985 ocorrências em MG), obtemos 2.579 transações após o subset; a base completa multi-ano dependerá dos CSVs 2023--2025 (ver \texttt{data/README.md}).
...
Após aplicar o filtro geográfico UF = MG, obtivemos 2.985 ocorrências (2026 parcial). Na preparação redesenhada (Pacote~A), selecionamos \textbf{8 colunas de contexto} (\texttt{condicao\_metereologica}, \texttt{tipo\_pista}, \texttt{tracado\_via}, \texttt{uso\_solo}, \texttt{causa\_acidente}, \texttt{tipo\_acidente}, \texttt{fase\_dia}, \texttt{fim\_de\_semana}) e o alvo \texttt{desfecho} $\in \{$Fatal, Ferido$\}$, excluindo \texttt{gravidade\_binaria}, \texttt{classificacao\_acidente}, \texttt{faixa\_horaria} e \texttt{dia\_semana} dos itens transacionais. Com filtro de frequência 0,5\%--99\%, obtemos 79 itens válidos entre 2.579 transações (com vítimas).

\paragraph{Resultado preliminar não trivial.}
Após filtrar regras com consequente \texttt{desfecho\_Fatal} e antecedentes exclusivamente contextuais, identificamos padrões interpretáveis, por exemplo: \{Transitar na contramão, Colisão frontal, Zona rural\} $\rightarrow$ Fatal (suporte 0,9\%, confiança 52,4\%, lift 6,40) --- alinhado parcialmente à hipótese H3.
...
\input{tabelas/top_tipos_acidente.tex}
...
\subsection{Modeling --- Implementado (Pacote A)}

A fase de Modeling foi reimplementada no módulo \texttt{src/} e no notebook \texttt{04\_modelagem.ipynb} (seção V2). A mineração restrita aplica FP-Growth com \texttt{min\_support=0,005}, filtra regras cujo consequente é \texttt{desfecho\_Fatal} e cujos antecedentes contêm apenas itens de contexto (prefixo \texttt{ctx\_}), poda regras não minimais e ranqueia por lift, leverage e conviction. Com dados de 2026 (parcial), obtivemos regras contextuais não tautológicas; a consolidação multi-ano (2023--2025) ampliará o suporte estatístico e a análise de estabilidade temporal.
